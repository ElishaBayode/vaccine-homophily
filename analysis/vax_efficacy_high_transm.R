...
beta = data.frame(beta=c(seq(0.3,0.5, length.out=15)))
...
plot_no_phil <- plot_output(data=long_form) + labs(y="Infections (without  homophily)", x ="Time (days)") + ylim(c(0,70000))
plot_phil <- plot_output(data=long_form_phil) + labs(y="Infections (with  homophily)", x ="Time (days)") + ylim(c(0,70000))

high_effic_high_transm <- ggarrange(plot_ratio , plot_phil, plot_no_phil,  
              labels = c("A", "B", "C"),
              ncol = 3, nrow = 1)

ggsave(file="figures/high_effic_high_transm.png",  high_effic_high_transm, width = 12, height = 6)
...
low_plot_no_phil <- plot_output(data=low_long_form) + labs(y="Infections (without homophily)", x ="Time (days)") + ylim(c(0,1500000))
low_plot_phil <- plot_output(data=low_long_form_phil) + labs(y="Infections (with  homophily)", x ="Time (days)")  + ylim(c(0,1500000))
...
low_effic_high_transm <- ggarrange(low_plot_ratio , low_plot_phil,  low_plot_no_phil, 
          labels = c("A", "B", "C"),
          ncol = 3, nrow = 1)

ggsave(file="figures/low_effic_high_transm.png",  low_effic_high_transm, width = 12, height = 6)
