...
beta = data.frame(beta=c(seq(0.5,0.7, length.out=15)))
...
plot_no_phil <- plot_output(data=long_form) + labs(y="Infections (without  homophily)", x ="Time (days)") + ylim(c(0,100000))
plot_phil <- plot_output(data=long_form_phil) + labs(y="Infections (with  homophily)", x ="Time (days)") + ylim(c(0,100000))

high_effic_vhigh_transm <- ggarrange(plot_ratio , plot_phil, plot_no_phil,  
                                    labels = c("A", "B", "C"),
                                    ncol = 3, nrow = 1)

ggsave(file="figures/high_effic_vhigh_transm.png",  high_effic_vhigh_transm, width = 12, height = 6)
...
low_plot_ratio <- plot_output(data= low_ratio_phil_no_phil ) + 
  labs(y="Ratio of Infections (with homophily to without homophily)", x ="Time (days)") +
  geom_hline(yintercept=1, linetype="solid", color = "black", size=0.5)  + ylim(c(0,70))

low_plot_no_phil <- plot_output(data=low_long_form) + labs(y="Infections (without homophily)", x ="Time (days)") + ylim(c(0,2500000))
low_plot_phil <- plot_output(data=low_long_form_phil) + labs(y="Infections (with  homophily)", x ="Time (days)")  + ylim(c(0,2500000))
...
low_effic_vhigh_transm <- ggarrange(low_plot_ratio , low_plot_phil,  low_plot_no_phil, 
                                   labels = c("A", "B", "C"),
                                   ncol = 3, nrow = 1)

ggsave(file="figures/low_effic_vhigh_transm.png",  low_effic_vhigh_transm, width = 12, height = 6)
